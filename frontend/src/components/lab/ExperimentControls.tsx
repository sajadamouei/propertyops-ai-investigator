    ...
    <section className="lab-controls panel-dark">
      <div className="controls-heading">
        ...
        <div className="run-actions">
          <button className="button button-ghost-dark" onClick={onReset}><RotateCcw size={15} /> Reset</button>
      ...
      <div className="control-grid">
        <label className="control-field control-wide"><span>Scenario</span><select value={config.scenario} onChange={(event) => setField('scenario', event.target.value as ExperimentScenario)}><option value="fault">Heating Valve Fault</option><option value="normal">Normal Operation</option><option value="custom">Custom Fault</option></select></label>
        <label className="control-field"><span>Days</span><input type="number" min="2" max="60" value={config.days} onChange={(event) => setField('days', Number(event.target.value))} /></label>
        <label className="control-field"><span>Random seed</span><input type="number" min="0" value={config.randomSeed} onChange={(event) => setField('randomSeed', Number(event.target.value))} /></label>
      ...
      {config.scenario === 'custom' && (
        <div className="custom-fault" aria-label="Custom fault specification">
          ...
          <div className="fault-grid">
            <label className="control-field"><span>Sensor</span><select value={config.customFault.sensorId} onChange={(event) => setFault('sensorId', event.target.value)}><option>AHU01-POWER</option><option>AHU01-HEAT-VALVE</option><option>AHU01-SUPPLY-TEMP</option><option>AHU01-FAN</option><option>ZONE03-TEMP</option></select></label>
            <label className="control-field"><span>Start</span><input type="datetime-local" value={config.customFault.start} onChange={(event) => setFault('start', event.target.value)} /></label>
            <label className="control-field"><span>End</span><input type="datetime-local" value={config.customFault.end} onChange={(event) => setFault('end', event.target.value)} /></label>
            <label className="control-field"><span>Fault type</span><select value={config.customFault.type} onChange={(event) => setFault('type', event.target.value as FaultType)}><option value="spike">Spike</option><option value="multiplier">Multiplier</option><option value="offset">Offset</option><option value="stuck">Stuck</option><option value="missing">Missing</option></select></label>
            <label className="control-field"><span>Value / parameter</span><input type="number" step="0.1" value={config.customFault.value} onChange={(event) => setFault('value', Number(event.target.value))} disabled={config.customFault.type === 'missing'} /></label>
          </div>
        </div>
      )}
    </section>
